...
function gameTick() {
  try {
    ...
    // spawn mobs at not crypto safe random intervals
    // increase the interval as the score goes up
    const spawnInterval = Math.ceil(200 / ((score + 1) / 3));
    if (curTick > START_TICK && curTick % spawnInterval === 0) {
      spawnMobs(MinecraftEntityTypes.Zombie);
    }

    if (curTick > START_TICK && curTick % 29 === 0) {
      addObstacles(MinecraftBlockTypes.CherryLeaves);
    }
  } catch (e) {
...
function spawnMobs(mobType: MinecraftEntityTypes) {
  const overworld = world.getDimension(MinecraftDimensionTypes.Overworld);
  ...
  for (let j = 0; j < spawnMobCount; j++) {
    ...
    overworld.spawnEntity(mobType, Vector3Utils.add(ARENA_VECTOR_OFFSET, { x: zombieX, y: 1, z: zombieZ }));
  }
}

function addObstacles(blockType: MinecraftBlockTypes) {
  const overworld = world.getDimension(MinecraftDimensionTypes.Overworld);
  for (let i = 0; i < 10; i++) {
